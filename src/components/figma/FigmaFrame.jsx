...
import { Link, Maximize2, Wifi, Battery, Signal } from 'lucide-react';
...
export const FigmaFrame = ({ step, isFocused, onFocus }) => {
  const { copyDirectLink, selectStep } = useApp();
  ...
  return (
    <div
      onClick={onFocus}
      className={`transition-all duration-300 relative select-none group flex flex-col items-center cursor-pointer`}
      style={{ width: '380px' }}
    >
      {/* Figma Frame Header Label */}
      <div
        className={`w-full flex items-center justify-between px-3.5 py-2 rounded-2xl text-xs font-bold mb-2 transition-all ${isFocused ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20 ring-2 ring-indigo-400' : 'bg-white/80 text-slate-700 hover:bg-white border border-indigo-100 shadow-sm'}`}
      >
      ...
      >
        ...
        <div className="h-8 bg-slate-950 px-6 flex items-center justify-between text-[11px] font-mono text-slate-400 select-none z-20 flex-shrink-0">
          <span>09:41</span>
        ...
        <div className="flex-1 overflow-y-auto bg-gradient-to-b from-indigo-50 via-purple-50 to-slate-50 text-slate-900 relative">
          {renderViewContent()}
        </div>
